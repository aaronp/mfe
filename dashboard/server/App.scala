...
def env(key: String, default: String = null) =
  def bang = sys.error(s"$key env variable not set: ${sys.env.mkString("\n", "\n", "\n")}\n properties:\n ${sys.props.mkString("\n")}")
...
def fetch(url: String) = {
  ...
  response.ensuring(_.statusCode == 200, s"${conf.listUrl} returned ${response.statusCode}: $response").text()
}

def listComponents(url: String = conf.listUrl): Seq[ListResponse] = read[List[ListResponse]](fetch(url))

object App extends cask.MainRoutes {


  extension (time : ZonedDateTime) {
    def elapsedSeconds(now :ZonedDateTime): Long = now.toInstant.toEpochMilli - time.toInstant.toEpochMilli
  }
    ...
    fileName match {
      case "bundle.js" =>
        cache.getJavascript(id) match {
          case Some(js) => cask.Response(data = js, headers = Seq("Access-Control-Allow-Origin" -> "*", "Content-Type" -> "application/javascript"))
          case None => cask.Response("not found", statusCode = 404)
        }
      case "bundle.css" =>
        cache.getCSS(id) match {
          case Some(css) => cask.Response(data = css, headers = Seq("Access-Control-Allow-Origin" -> "*", "Content-Type" -> "text/plain"))
          case None => cask.Response("not found", statusCode = 404)
        }
      case other => cask.Response(s"ignoring '$other'. specify bundle.js or bundle.css, e.g /component/$id/bundle.js", statusCode = 501)
    }
  }
  ...
  println(s""" 🚀 running Dashboard BFF on $host:$port {verbose : $verbose, debugMode : $debugMode }  🚀""")
}
